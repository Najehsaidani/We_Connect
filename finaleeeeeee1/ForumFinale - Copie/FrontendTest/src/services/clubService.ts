...
export const clubService = {
  ...
  getClubById: async (id: number): Promise<ClubDto> => {
    try {
      ...
      console.warn(`Erreur lors de la récupération du club ${id}, affichage d'un club fictif.`);

      // Trouver un club fictif avec l'ID correspondant
      const mockClubs = clubService.getMockClubs();
      const mockClub = mockClubs.find(club => club.id === id);

      // Si un club fictif avec cet ID existe, le renvoyer
      if (mockClub) {
        return mockClub;
      }

      // Sinon, créer un nouveau club fictif avec cet ID
      return {
        id: id,
        nom: `Club ${id}`,
        description: "Description du club fictif",
        categoryId: 1,
        category: { id: 1, nom: "Académique" },
        createurId: 6,
        etat: "ACCEPTER",
