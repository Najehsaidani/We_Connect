...
export const userService = {
  ...
  getUserByEmail: async (email: string): Promise<User> => {
    try {
      ...
      // Try both endpoints to handle potential API differences
      try {
        ...
        const response = await apiClient.get(`/users/email?email=${encodeURIComponent(email)}`);
        console.log('User data response from alternative endpoint:', response.data);
        return response.data;
      }
    } catch (error) {
