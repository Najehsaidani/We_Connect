...
export interface PostResponse {
  ...
  authorAvatar?: string;  // Ajout de l'avatar de l'auteur
  content: string;
  category: string;
  imageUrl?: string;
  image?: string;  // Added image property to match what's used in the AdminPage
  createdAt?: string;
  updatedAt?: string;
  userId?: number;  // Ajout de l'ID de l'utilisateur
}
...
export const PostsService = {
  // Create a new post
  createPost: async (post: PostRequest, userId: number): Promise<PostResponse> => {
    try {
      console.log("Creating post with data:", post, "and userId:", userId);

      // Add userId as a query parameter
      const res = await apiClient.post(`/posts?userId=${userId}`, post);
