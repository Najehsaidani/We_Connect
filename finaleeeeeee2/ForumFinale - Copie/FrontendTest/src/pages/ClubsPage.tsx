...
const ClubsPage = () => {
    ...
    try {
      ...
      const convertedClubs: Club[] = data.map(dto => {
        ...
        return {
          ...
          creatorId: Number(dto.createurId || userId || 1),
          ...
          status: dto.etat || 'EN_ATTENTE',
          etat: dto.etat || 'EN_ATTENTE'
        };
      });
  ...
  const handleCreateClub = async () => {
    ...
    try {
      ...
      alert(`Le club a été créé avec succès en mode simulation. Cependant, le backend a rencontré une erreur 500 (Internal Server Error). Veuillez contacter l'administrateur système pour résoudre ce problème.`);

      // Rafraîchir la liste des clubs pour afficher le club fictif
      await fetchClubs();
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredClubs = clubs.filter((club) => {
    ...
    const isAccepted = clubStatus === 'ACCEPTER' || clubStatus === 'accepter';
  ...
  const userCreatedClubs = clubs.filter(club => {
    ...
    return creatorId === currentUserId && !isNaN(creatorId) && !isNaN(currentUserId);
  });
        ...
        <>
          ...
          <ClubsList
            ...
            onClubDetailsOpen={(club) => {
              ...
              const creatorId = Number(club.creatorId);
              const currentUserId = Number(userId);
              const isCreator = creatorId === currentUserId && !isNaN(creatorId) && !isNaN(currentUserId);
