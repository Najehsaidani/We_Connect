...
const EventsPage = () => {
  ...
  const [clubEvents, setClubEvents] = useState<FormattedEvent[]>([]);
      ...
      try {
        if (newEvent.type === 'Event Club') {
          ...
          formattedEvent = {
            ...
            type: 'Event Club' as const,
            isUpcoming: true,
            createurId: currentUserId, // Ajouter l'ID du créateur pour les permissions
            source: 'club' as const // Ajouter la source pour le filtrage
          };
          ...
          formattedEvent = {
            ...
            organizer: 'Vous',
            attending: createdEvent.nbParticipants || 1,
            type: 'Event Universitaire' as const,
            isUpcoming: true,
            createurId: currentUserId, // Ajouter l'ID du créateur pour les permissions
            source: 'university' as const // Ajouter la source pour le filtrage
          };
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          formattedEvent = {
            ...
            organizer: 'Vous',
            attending: createdEvent.nbParticipants || 1,
            type: newEvent.type === 'Event Club' ? 'Event Club' as const : 'Event Universitaire' as const,
            isUpcoming: true,
            createurId: currentUserId, // Ajouter l'ID du créateur pour les permissions
            source: newEvent.type === 'Event Club' ? 'club' as const : 'university' as const // Ajouter la source pour le filtrage
          };
  ...
  // Handle delete event
  const handleDeleteEvent = (event: FormattedEvent) => {
    setSelectedEvent(event);
