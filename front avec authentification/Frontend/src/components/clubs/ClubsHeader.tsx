...
interface ClubsHeaderProps {
  categories: string[]; // ex: ['IT', 'Commerce', 'Social']
  activeCategory: string; // ex: 'IT' ou ''
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        {/* Boutons de catégorie (avec "Tous") */}
        <div className="flex overflow-x-auto pb-2 space-x-2 w-full md:w-auto">
          <button
            key="all"
            onClick={() => onCategoryChange('')}
            className={`px-4 py-2 rounded-full whitespace-nowrap ${
              activeCategory === '' ? 'bg-primary text-white' : 'bg-muted hover:bg-muted/80'
            }`}
          >
            Tous
          </button>
