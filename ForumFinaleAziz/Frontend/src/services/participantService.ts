...
//       console.log('API URL:', apiClient.defaults.baseURL + '/participants/join');

//       // Utiliser l'endpoint correct du backend
//       // Cet endpoint est défini dans ParticipantController.java
...
//       } else {
//         console.error('Empty response data when joining event');
...
//       console.log('API URL:', apiClient.defaults.baseURL + '/participants/leave');

//       // Use the correct endpoint from your backend controller
//       const response = await apiClient.delete('/participants/leave', {
//         params: {
//           userId: Number(userId),
//           eventId: Number(eventId)
//         },
...
//       console.log('Leave event response status:', response.status);
...
//       try {
//         console.log('Trying alternative endpoint for leaving event');
//         await apiClient.post('/participants/cancel', null, {
//           params: {
//             userId: Number(userId),
//             eventId: Number(eventId)
//           },
...
//       try {
//         // Cet endpoint est défini dans ParticipantController.java
//         console.log(`API URL for user events: ${apiClient.defaults.baseURL}/participants/user/${userId}`);
...
//         console.log('User events response status:', response.status);
//         console.log('Raw user events data:', response.data);

//         // Vérifier si les données sont dans le format attendu
//         if (!response.data) {
//           console.error('No data received for user events');
//           return [];
//         }
...
//             }
//           } else {
//             console.error('Invalid data format for user events');
//             return [];
//           }
//         }

//         console.log(`Processing ${participantsData.length} participants`);

//         // Convertir les données de participation en événements
//         // Les données retournées sont des objets Participant, pas des Event
...
//               console.log('Found event in participant:', participant.event);
//               // Vérifier si l'événement a toutes les propriétés nécessaires
//               const event = participant.event;
//               if (!event.id && event.id !== 0) {
//                 console.warn('Event has no ID, using participant eventId');
//                 event.id = participant.eventId || 0;
//               }
//               return event;
//             } else if (participant.eventId || participant.eventId === 0) {
//               console.log('Found eventId in participant:', participant.eventId);
//               // Si l'événement n'est pas inclus dans la réponse, créer un objet minimal
//               return {
//                 id: participant.eventId,
//                 titre: participant.eventTitle || 'Événement',
...
//               };
//             } else {
//               console.warn('Participant has no event or eventId:', participant);
//               return null;
//             }
//           } catch (mapError) {
//             console.error('Error mapping participant to event:', mapError, participant);
//             return null;
//           }
...
//         return events;
//       } catch (innerError) {
//         console.error('Error fetching user events:', innerError);
...
//         try {
//           console.log('Trying alternative endpoint for user events');
//           const altResponse = await apiClient.get(`/users/${userId}/events`, {
//             timeout: 15000,
//             headers: {
//               'Accept': 'application/json',
//               'Content-Type': 'application/json'
...
//             if (Array.isArray(altResponse.data.events)) {
//               return altResponse.data.events;
//             }
//           }
//         } catch (altError) {
//           console.error('Alternative method also failed:', altError);
