...
const handleCancelEvent = async (eventId: number): Promise<void> => {
  ...
  try {
    ...
    // Nous allons utiliser une approche simplifiée pour l'annulation
    console.log("Using simplified approach for event cancellation");
    ...
    if (isClubEvent) {
      try {
        // Utiliser la fonction spécifique pour annuler un événement de club
        console.log("Using specialized cancelEvent function for club event");
        const result = await eventsClubsService.cancelEvent(eventId, adminId);
        updatedEvent = { id: result.id || eventId, ...result };
      } catch (error) {
        console.error("Error with cancelEvent, using simple status update:", error);

        // En cas d'échec, essayer avec une mise à jour simple du statut
        const result = await eventsClubsService.updateEvent(eventId, { status: "INACTIVE" }, adminId);
        updatedEvent = { id: result.id || eventId, ...result };
      }
    } else {
      // Pour les événements standard, utiliser la mise à jour simple du statut
      const result = await EventsService.updateEvent(eventId, { status: "INACTIVE" }, adminId);
      updatedEvent = { id: result.id || eventId, ...result };
    }
        ...
        {selectedClub && (
          <div className="py-4">
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="edit-club-description" className="text-sm font-medium">Description</label>
                <textarea
